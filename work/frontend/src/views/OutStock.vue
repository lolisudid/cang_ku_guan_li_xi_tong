  ...
  <el-card>
    <template #header>
      <div style="display:flex; justify-content:space-between;">
        <span>出库管理</span>
        <el-button type="primary" size="small" @click="openDialog">新增出库</el-button>
      </div>
    </template>
    <el-table :data="list" border stripe v-loading="loading">
      <el-table-column prop="id" label="出库单号" width="110" />
    ...
    <el-dialog v-model="dialogVisible" title="新增出库单" width="500px">
      <el-form :model="form" label-width="100px">
        <el-form-item label="出库单号"><el-input v-model="form.id" /></el-form-item>
        ...
        <el-form-item label="出库数量"><el-input-number v-model="form.quantity" :min="1" style="width:100%;" /></el-form-item>
      </el-form>
      <template #footer>
...
import { getOutStock, createOutStock } from '../api'
...
async function fetchData() {
  loading.value = true
  try {
    const res = await getOutStock({ page: page.value - 1, size: size.value })
...
function openDialog() {
  form.value = { id: `O${Date.now()}`, goodsId: '', warehouseId: '', quantity: 1 }
...
async function handleSave() {
  ...
  try {
    ...
    await createOutStock(form.value)
